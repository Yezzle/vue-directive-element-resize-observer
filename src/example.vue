<template>
  <div class="example-container">
    <h1>Vue Directive Element Resize Observer 示例</h1>
...
import type { SizableClassRule, ResizeInfo } from './types'

// 响应式数据
const currentWidth = ref(0)
const appliedClasses = ref<string[]>([])
const breakpoints = ref([480, 768, 1024])

// 回调函数相关数据
const resizeInfo = ref<ResizeInfo>({
  width: 0,
  height: 0,
...
const aspectRatio = computed(() => {
  if (resizeInfo.value.height === 0) return 0
  return resizeInfo.value.width / resizeInfo.value.height
})
...
const elementStatus = computed(() => {
  const { width, height } = resizeInfo.value
...
const handleCombinedResize = (info: ResizeInfo) => {
  combinedWidth.value = info.width
  combinedHeight.value = info.height
  callbackCount.value++
}

const handleDynamicResize = (info: ResizeInfo) => {
  dynamicWidth.value = info.width
    ...
    mutations.forEach((mutation) => {
      if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
        ...
        if (target.classList.contains('responsive-demo')) {
          appliedClasses.value = Array.from(target.classList).filter(cls => 
...
onMounted(() => {
  ...
  classObserver = observeClassChanges()
})
